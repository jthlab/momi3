...
def test_Qmut(axes, n, dims, rng):
    Qmut0 = Integration._calcB([int(dims[i] - 1) for i in range(len(dims))], 1.0)
    pops = [("A", "B")]
    aux = lift_cm_aux(axes, pops)
    mig_mat = {pops[0]: 0.01}
    _, Qmut1 = _Q_mig_mut(dims, axes, mig_mat, aux)
    M0, M1 = [np.asarray(x.todense()) for x in (Qmut0, Qmut1.materialize())]
...
def test_lift_eq_exp_m0(rng):
    ...
    for pop in axes:
        ...
        plp_nomig, e = _lift1(
            plp_nomig, i, etas[pop], t[0], t[1], **aux, terminal=False
        )
        sl = [0] * 2
        sl[i] = slice(None)
